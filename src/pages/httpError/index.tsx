...
const httpError: React.FC = observer((props) => {
  ...
  const tabTitle = (<>
    <span className="fs-18">接口请求失败列表</span><span>（Top20）</span>
  </>)
  let times = '<1秒'
        ...
        renderItem={item =>
          <List.Item onClick={toDetail}>
            <div className="fs-12 flex wd_100s">
              <Button >{item.type}</Button><span className="ml-4">{item.name}</span><AndroidFilled className="ml-4" />（{item.android}次）<AppleFilled className="ml-4" />（{item.iphone}次）<WindowsFilled className="ml-4" /><span>（{item.man}次）</span><div className="flex-right color-info">最近：{item.time}</div> <RightOutlined className="ml4" />
            </div>
  ...
  const jsTitle = (
    <div><span>接口请求报错</span><span>(点击柱状图查看其它日期)</span></div>
  )
  const selfTitle = (
